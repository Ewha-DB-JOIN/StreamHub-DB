...
| 분석 ① | [REQ13] 플랜 ID 입력 → 가격 변동 전후 매출 비교 분석 | 신우림 |
...
| 이름 | 담당 테이블 | 담당 메뉴 | 기타 |
|------|------------|---------|------|
| 신우림 | `price_history` | 분석① (REQ13) | 스키마 통합·검수, 뷰 2개·인덱스 작성, Main.java 메뉴 틀, SQL 3종 최종 정리·제출, README, 보고서 취합 |
| 박나림 | `member` + `member_profile_history` | DELETE① (회원 탈퇴), 분석② (REQ14) | 회원 인적사항 이력 관리 구현, 발표자료 제작 |
| 최보경 | `content` | INSERT① (콘텐츠 등록), DELETE② (콘텐츠 삭제), SELECT④ (장르별 집계) | 초기 데이터 50건, 데모 영상 제작 |
| 조수민 | `subscription_plan` | UPDATE① (가격 변경, 트랜잭션) | REQ12 트랜잭션 처리 담당, 발표 |
| 이태영 | `subscription` | INSERT② (구독 등록), UPDATE② (플랜 변경) | region_snapshot 스냅샷 처리, 발표 |
| 곽성은 | `watch_history` | SELECT① (장르별 시청 통계, VIEW+JOIN), SELECT③ (월별 매출 집계) | billing 테이블 초기 데이터 지원, 발표자료 제작 |
| 하지수 | `billing` | SELECT② (회원별 결제 이력, VIEW+JOIN) | PreparedStatement 전체 검수, .jar 빌드, 데모 영상 제작 |
...
| **5/30** | **제안서 제출** | 신우림 |
| 6/01 ~ 6/05 | SQL 작성 (createschema.sql, initdata.sql — 각자 담당 테이블) | ALL |
| 6/01 ~ 6/07 | Java 메뉴 구현 (각자 담당 REQ) | ALL |
| 6/07 | develop 브랜치 통합 및 통합 테스트 | 신우림 |
| 6/08 | .jar 빌드, 보고서 초안 완성 | 하지수, 신우림 |
| **6/09** | **최종 제출** | 신우림 |
| 6/10 | 발표 자료 및 데모 영상 준비 | 박나림, 최보경, 곽성은, 하지수 |
| **6/11** | **발표 및 데모** | 이태영, 조수민 |
...
| **IDE** | IntelliJ IDEA / Eclipse / Antigravity |
| **형상 관리** | GitHub [Ewha-DB-JOIN/StreamHub-DB](https://github.com/Ewha-DB-JOIN/StreamHub-DB) |
| **협업·이슈 관리** | [GitHub Projects](https://github.com/orgs/Ewha-DB-JOIN/projects/1), [GitHub Issues](https://github.com/Ewha-DB-JOIN/StreamHub-DB/issues) (REQ1~20), Notion |
| **ERD 설계** | [dbdiagram.io](https://dbdiagram.io/d/2026-1-Database-StreamHub-6a0ab63f9f1f8ec47b3a5aa2) |
| **문서 작성** | [Google Docs](https://drive.google.com/drive/folders/1mFwR-SqegUhyA8bl5sCKK7uyYA8gCzC9?usp=drive_link) (제안서·보고서), Markdown (README), Overleaf |
